...
// -- Working-day helpers (mirrors HolidayEngine logic) --
...
export const TaskEditModal: React.FC<TaskEditModalProps> = ({ task, onClose }) => {
  ...
  const [taskColor, setTaskColor] = useState(task.color || project?.config.defaultBarColor || BAR_COLORS[0].value);
  ...
  const handleSave = useCallback(() => {
    const finalDur = durStr ? parseInt(durStr) || 1 : task.duration;
    updateTask(task.id, {
      name,
      duration: finalDur,
    ...
    onClose();
  }, [name, durStr, manualStartDate, taskColor, taskStatus, taskAssignee, taskNotes, task.id, task.duration, updateTask, onClose]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm animate-fadeIn">
      <div
        ref={modalRef}
        className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-[480px] max-h-[90vh] overflow-y-auto animate-scaleIn"
      >
        {/* Header */}
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-5 gap-3 items-end">
            ...
            <div className="col-span-2 space-y-2">
              ...
              <input
                ...
                className="w-full px-3.5 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all [color-scheme:light]"
              />
            </div>
          </div>
          ...
          {/* Color */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">สีแถบงาน</label>
            <div className="flex items-center gap-1.5 flex-wrap">
              {BAR_COLORS.map(c => (
                <button
                  key={c.value}
                  onClick={() => { if (taskColor !== c.value) setTaskColor(c.value); }}
                  className={`w-7 h-7 rounded-full border-2 transition-all hover:scale-110 ${taskColor === c.value ? 'border-slate-800 scale-110 shadow-md' : 'border-transparent'}`}
                ...
                <button
                  onClick={() => {
                    ...
                    const updatedTasks = p.tasks.map(t =>
                      t.id === task.id
                        ? { ...t, color: taskColor, name, duration: finalDur, manualStartDate: manualStartDate || undefined, status: taskStatus, assignee: taskAssignee || undefined, notes: taskNotes || undefined }
                        : { ...t, color: taskColor }
                    );
                    updateProject({
                      config: { ...p.config, defaultBarColor: taskColor },
                      tasks: updatedTasks,
                    });
                    onClose();
                  }}
                  className="flex-1 px-3 py-2 bg-slate-100 hover:bg-blue-100 text-slate-700 hover:text-blue-700 text-xs font-bold rounded-lg transition-all active:scale-95"
                >
                  ใช้สีนี้ทั้งหมด
                </button>
                <button
                  ...
                  className="flex-1 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg transition-all active:scale-95"
                >
                  ใช้สีนี้เฉพาะอันนี้
                </button>
              </div>
        ...
        <div className="flex items-center justify-between p-6 border-t border-slate-100">
          ...
          <button
            onClick={handleSave}
            className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-600/20 transition-all active:scale-[0.98]"
          >
            บันทึก
          </button>
        </div>
